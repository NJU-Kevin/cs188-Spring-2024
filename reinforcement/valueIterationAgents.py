...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            # For every state, update corresponding value
            states = self.mdp.getStates()
            updateValues = util.Counter()
            for state in states:
                actions = self.mdp.getPossibleActions(state)
                if self.mdp.isTerminal(state) or not actions:
                    updateValues[state] = 0
                else:
                    qValues = [self.computeQValueFromValues(state, action) 
                                for action in actions]
                    updateValues[state] = max(qValues)
            # After each iteration, update self.values
