...
"""I make two additional PQWF classes so as to apply to UCS and aStar"""
class PriorityQueueWithFunction_UCS(PriorityQueue):
    """
    Implements a priority queue with the same push/pop signature of the
    Queue and the Stack classes. This is designed for drop-in replacement for
    those two classes. The caller has to provide a priority function, which
    ...
    def push(self, item):
        ...
        """I change the following to make the priorityFunction apply to the
        item in search.py, since I push a tuple (state, action) there and the
        priorityFunction is applied to action lists rather than a tuple"""
        PriorityQueue.push(self, item, self.priorityFunction(item[1]))

class PriorityQueueWithFunction_AStar(PriorityQueue):
    ...
    def push(self, item):
        ...
        heuristic_val = self.priorityFunction(state, self.problem)
        total_cost = cost + heuristic_val
        PriorityQueue.push(self, item, total_cost)
