...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Pacman and the two ghosts can be anywhere in the grid (we ignore walls for 
    # this). Add all possible position tuples for these.
    allCoords = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[PAC] = allCoords
    variableDomainsDict[GHOST0] = allCoords
    variableDomainsDict[GHOST1] = allCoords
    # Observations here are non-negative, equal to Manhattan distances of Pacman 
    # to ghosts ± noise.
    maxDistance = X_RANGE + Y_RANGE - 2 + MAX_NOISE
    maxObsDomain = list(range(maxDistance + 1))
    variableDomainsDict[OBS0] = maxObsDomain
    variableDomainsDict[OBS1] = maxObsDomain
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # iterate by eliminationOrder
        for eliminateVariable in eliminationOrder:
            # join @eliminateVariable in currentFactorsList
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminateVariable)
            # Now @currentFactorsList does not contain factors containing 
            # @eliminateVariable, and @joinedFactor is the result of joining 
            # @eliminateVariable
            if len(joinedFactor.unconditionedVariables()) > 1:
                # Next, if joinedFactor has more than 1 unconditionedVariables, 
                # eliminate @eliminateVariable from @joinedFactor and 
                # append the result to @currentFactorsList
                currentFactorsList.append(eliminate(joinedFactor, eliminateVariable))

        # currentFactorsList should contain the connected components of the 
        # graph now as factors, must join the connected components
        fullJoint = joinFactors(currentFactorsList)
        # Finally normalize the factor @fullJoint
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # ghostPosition == jailPosition <==> noisyDistance is None
        if ghostPosition == jailPosition and noisyDistance is None:
            return 1.0
        elif (ghostPosition == jailPosition and noisyDistance is not None) or (ghostPosition != jailPosition and noisyDistance is None):
            return 0.0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # location, and are stored as a DiscreteDistribution object in a field 
        # called self.beliefs, which you should update.
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        for coordinate in self.allPositions:
            coordProb = self.getObservationProb(observation, pacmanPosition, 
                                                coordinate, jailPosition)
            self.beliefs[coordinate] *= coordProb
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initialize self.beliefs since we should update it from 0 every time
        oldBelief = self.beliefs.copy()
        for coordinate in self.beliefs.keys():
            self.beliefs[coordinate] = 0
        # P(ghost at newPos) = P(ghost at oldPos) * P(ghost at newPos | ghost at oldPos)
        # Then sum over oldPos, this is why each update should start from 0
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in newPosDist.keys():
                self.beliefs[newPos] += oldBelief[oldPos] * newPosDist[newPos]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        numOfLegalPositions = len(self.legalPositions)
        # Using mod operator, view 0, 1, ..., numOfParticles - 1 as 
        # @numOfParticles particles, and initialize them uniformly in all 
        # possible positions
        for i in range(numOfParticles):
            self.particles.append(self.legalPositions[i % numOfLegalPositions])
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        for coordinate in self.particles:
            beliefs[coordinate] += 1
        beliefs.normalize()
        return beliefs
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        numOfParticles = len(self.particles)
        ...
        for particlePosition in self.particles:
            weight = self.getObservationProb(observation, pacmanPosition,
                                             particlePosition, jailPosition)
            weightDistribution[particlePosition] += weight
        if weightDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weightDistribution.sample() 
