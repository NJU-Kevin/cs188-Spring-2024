...
Batch size: between 1 and 128. For Q2 and Q3, we require that total size of the 
    dataset be evenly divisible by the batch size.
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        self.layer1 = Linear(input_size, self.hidden_layer1)
        self.layer2 = Linear(self.hidden_layer1, self.hidden_layer2)
        self.layer3 = Linear(self.hidden_layer2, output_size)
...
def Convolve(input: tensor, weight: tensor):
    ...
    "*** YOUR CODE HERE ***"
    intputHeight, inputWidth = input_tensor_dimensions
    weightHeight, weightWidth = weight_dimensions
    ouputHeight = intputHeight - weightHeight + 1
    outputWidth = inputWidth - weightWidth + 1
    Output_Tensor = torch.zeros(ouputHeight, outputWidth)
    for i in range(ouputHeight):
        for j in range(outputWidth):
            window = input[i : i + weightHeight, j : j + weightWidth]
            Output_Tensor[i, j] = tensordot(window, weight)
    "*** End Code ***"
...
class DigitConvolutionalModel(Module):
    ...
    def run(self, x):
        ...
        x = stack(list(map(lambda sample: 
                        Convolve(sample, self.convolution_weights), x)))
