...
def sentence2() -> Expr:
    ...
    expr3 = (~(B & ~C)) >> A
    expr4 = ~D >> C
    result = conjoin([expr1, expr2, expr3, expr4])
    return result
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    alive1 = PropSymbolExpr("PacmanAlive", time=1)
    alive0 = PropSymbolExpr("PacmanAlive", time=0)
    born0 = PropSymbolExpr("PacmanBorn", time=0)
    killed0 = PropSymbolExpr("PacmanKilled", time=0)
    expr1 = alive1 % ((alive0 & ~killed0) | (~alive0 & born0))
    expr2 = ~(alive0 & born0)
    expr3 = born0
    result = conjoin([expr1, expr2, expr3])
    return result
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # the result does not hold iff premise is true and conclusion is false
    expr = premise & ~conclusion
    return not findModel(expr)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for (i, j) in itertools.combinations(range(n), 2):
        clauses.append(disjoin([~literals[i], ~literals[j]]))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coordinate in all_coords:
        (x, y) = coordinate
        isWall = PropSymbolExpr(wall_str, x, y)
        noPacman = ~PropSymbolExpr(pacman_str, x, y, time=t)
        expr1 = isWall >> noPacman
        pacphysics_sentences.append(expr1)
    # 2nd
    # Pacman is at exactly one of the squares at timestep t.
    pacmanAt = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in
                 non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(pacmanAt))
    # 3rd
    # Pacman takes exactly one action at timestep t.
    pacmanDirection = [PropSymbolExpr(direction, time=t) for direction in
                        DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(pacmanDirection))
    # 4th
    # Results of calling sensorModel(...), unless None.
    if sensorModel:
    ...
    if successorAxioms and t > 0:
        pacphysics_sentences.append(successorAxioms(t, walls_grid,
                                                     non_outer_wall_coords))
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    non_outer_wall_coords = list(itertools.product(range(1, problem.getWidth() 
                                    + 1), range(1, problem.getHeight() + 1)))
    KB = []
    x0, y0 = x0_y0
    x1, y1 = x1_y1
    ...
    KB.append(pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid,
                                successorAxioms=allLegalSuccessorAxioms))
    # Pacman’s current location at (x0,y0)
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    # Pacman takes action0
    KB.append(PropSymbolExpr(action0, time=0))
    # Pacman takes action1
    KB.append(PropSymbolExpr(action1, time=1))
    pacmanAt = PropSymbolExpr(pacman_str, x1, y1, time=1)
    pacmanNotAt = ~pacmanAt
    model1 = findModel(conjoin(KB + [pacmanAt]))
    model2 = findModel(conjoin(KB + [pacmanNotAt]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # locations in non_wall_coords at timestep t
        pacmanAt = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in
                 non_wall_coords]
        KB.append(exactlyOne(pacmanAt))
        # Goal Assertion is the expression asserting that Pacman is at the goal
        #  at timestep
        atGoal = PropSymbolExpr(pacman_str, xg, yg, time=t)
        # Use findModel and pass in the Goal Assertion and KB
        model = findModel(conjoin(KB + [atGoal]))
        if model:
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        pacmanAt = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in
                 non_wall_coords]
        KB.append(exactlyOne(pacmanAt))
        # Change the goal assertion: your goal assertion sentence must be true 
        # if and only if all of the food have been eaten
        atGoal = [~PropSymbolExpr(food_str, x, y, time=t) for (x, y) in food]
        # Use findModel and pass in the Goal Assertion and KB
        model = findModel(conjoin(KB + atGoal))
        if model:
            return extractActionSequence(model, actions)
        ...
        # Here (x, y) should be in the list "food"  
        for (x, y) in food:
            KB.append(foodSuccessorAxiom(x, y, t))
    "*** END YOUR CODE HERE ***"

"*** BEGIN HELPER FUNCIONS HERE ***"
"""foodSuccessorAxiom helper function for method @foodLogicPlan"""
def foodSuccessorAxiom(x: int, y: int, t: int) -> Expr:
    # what is the relation between Food[x,y]_t+1 and Food[x,y]_t and 
    # Pacman[x,y]_t?
    pacmanPos = PropSymbolExpr(pacman_str, x, y, time=t)
    curFood = PropSymbolExpr(food_str, x, y, time=t)
    nextFood = PropSymbolExpr(food_str, x, y, time=t + 1)
    # ~nextFood <==> ~curFood or (curFood & pacmanPos) 
    expr = ~nextFood % (~curFood | (pacmanPos & curFood)) 
    return expr

"""Add pacphysics, action, and percept information to KB"""
def addInfo(agent, perceptsMap, KB: List, t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None):
    """Add to KB: pacphysics_axioms(...), which you wrote in q3. Use
      sensorAxioms and allLegalSuccessorAxioms for localization and mapping, and
     SLAMSensorAxioms and SLAMSuccessorAxioms for SLAM only."""
    KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid,
                                sensorModel, successorAxioms))
    # Add to KB: Pacman takes action prescribed by agent.actions[t]
    KB.append(PropSymbolExpr(agent.actions[t], time=t))
    """ Get the percepts by calling agent.getPercepts() and pass the percepts to 
    fourBitPerceptRules(...) for localization and mapping, or 
    numAdjWallsPerceptRules(...) for SLAM. Add the resulting percept_rules to KB.
    """
    perceptRules = perceptsMap(t, agent.getPercepts())
    KB.append(perceptRules)

# Find possible pacman locations with updated KB
def findPossibleLoc(KB: List, t: int, non_outer_wall_coords: List[Tuple], possible_locations: List):
    for (x, y) in non_outer_wall_coords:
        """Can we prove whether Pacman is at (x,y)? Can we prove whether Pacman 
        is not at (x,y)? Use entails and the KB. If there exists a satisfying 
        assignment where Pacman is at (x,y) at time t, add (x, y) to 
        possible_locations."""
        pacmanLoc = PropSymbolExpr(pacman_str, x, y, time=t)
        if findModel(conjoin(KB + [pacmanLoc])):
            possible_locations.append((x, y))
        # Add to KB: (x,y) locations where Pacman is provably at, at time t
        if (entails(conjoin(KB), pacmanLoc)):
            KB.append(pacmanLoc)
        if (entails(conjoin(KB), ~pacmanLoc)):
            KB.append(~pacmanLoc)

# Find provable wall locations with updated KB
def findProvableWallLoc(KB: List, known_map: List[List], non_outer_wall_coords: List[Tuple]):
    for (x, y) in non_outer_wall_coords:
        """Add to KB and update known_map: (x,y) locations where there is 
        provably a wall."""
        wallLoc = PropSymbolExpr(wall_str, x, y)
        if (entails(conjoin(KB), wallLoc)):
            KB.append(wallLoc)
            known_map[x][y] = 1
        """Add to KB and update known_map: (x,y) locations where there is 
        provably not a wall"""
...
def localization(problem, agent) -> Generator:
    ...
    all_coords = list(itertools.product(range(problem.getWidth() + 2), 
                                        range(problem.getHeight()+2)))
    non_outer_wall_coords = list(itertools.product(range(1, 
                problem.getWidth() + 1), range(1, problem.getHeight() + 1)))
    ...
    for (x, y) in all_coords:
        ...
        if (x, y) in walls_list:
            ...
            KB.append(~wallLoc)
    perceptsMap = fourBitPerceptRules
    sensorModel = sensorAxioms
    successorAxioms = allLegalSuccessorAxioms
    for t in range(agent.num_timesteps):
        # Add pacphysics, action, and percept information to KB
        addInfo(agent, perceptsMap, KB, t, all_coords, non_outer_wall_coords,
                 walls_grid, sensorModel, successorAxioms)
        # Find possible pacman locations with updated KB
        possible_locations = []
        findPossibleLoc(KB, t, non_outer_wall_coords, possible_locations)
        # Call agent.moveToNextState(action_t) on the current agent action at
        #  timestep t
...
def mapping(problem, agent) -> Generator:
    ...
    initialPacmanLoc = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)
    initialWallLoc = PropSymbolExpr(wall_str, pac_x_0, pac_y_0)
    KB.append(initialPacmanLoc)
    KB.append(~initialWallLoc)
    perceptsMap = fourBitPerceptRules
    sensorModel = sensorAxioms
    successorAxioms = allLegalSuccessorAxioms
    for t in range(agent.num_timesteps):
        # Add pacphysics, action, and percept information to KB.
        addInfo(agent, perceptsMap, KB, t, all_coords, non_outer_wall_coords,
                 known_map, sensorModel, successorAxioms)
        # Find provable wall locations with updated KB.
        findProvableWallLoc(KB, known_map, non_outer_wall_coords)
        #  Call agent.moveToNextState(action_t) on the current agent action at 
        # timestep t
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
...
def slam(problem, agent) -> Generator:
    ...
    perceptsMap = numAdjWallsPerceptRules
